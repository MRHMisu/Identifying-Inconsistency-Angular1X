module.exports.getInconsistency=getInconsistency;
...
function getInconsistency(directoryPath) {
    var path = "D:\\Implementation Work\\PapperDemoApp\\Shopping List App";//directoryPath;
    var MVCGroupList = MVCGroupBuilder.getMVCGroupList(path);
    var MVCGroup = MVCGroupList.MVCGroup;
    var MVCGroupForDirective = MVCGroupList.MVCGroupForDirective;
    checkInconsistency(MVCGroup);
    checkInconsistency(MVCGroupForDirective);
    printInconsistency();
}

function checkInconsistency(MVCGroupList) {
...
function printInconsistency()
{
    console.log('Model Variable Inconsistency'.black.bgWhite);
    for(var i=0; i<mv_inconsistencyList.length;i++)
    {
        mv_inconsistencyList[i].print(i);
    }
    console.log('Controller Function Inconsistency'.black.bgWhite);
    for(var j=0; j<cf_inconsistencyList.length;j++)
    {
        cf_inconsistencyList[j].print(j);
    }
}
        ...
        for (var j = 0; j < modelVariableInView.length; j++) {
            ...
            if (controllerModelVariable.modelVariableName.indexOf(viewModelVariable.modelVariableName) > -1) {
                ...
                if (!(controllerModelVariable.modelVariableName === viewModelVariable.modelVariableName)) {
                    ...
                    if (!(viewModelVariable.dataType === 'na' || controllerModelVariable.dataType == "Object" || controllerModelVariable.dataType == "functionCall")) {
        ...
        for (var j = 0; j < CFInView.length; j++) {
            ...
            if (cFINController.controllerFunctionName.indexOf(cFINView.controllerFunctionName) > -1) {
                if (!(cFINController.controllerFunctionName === cFINView.controllerFunctionName)) {
                    ...
                    var whereOccured = cFINController.controllerFunctionName + " >> line no: " + cFINView.lineNumber + " and " + controllerName + " >> line no: " +
                        cFINController.startLineNumber + " to " + cFINController.endLineNumber;
                    cf_inconsistencyList.push(new Inconsistency_Entity.inconsistency(type, occurence, whereOccured));
