...
function getJavaScriptCode() {
    ...
    var ast = esprima.parse(textInEditor);
    ...
    if (node.type == 'AssignmentExpression') {
        if (node.operator == '=') {
            if (node.left.object && node.left.type == 'MemberExpression') {
                ...
                var viewModelIdentifier = objectName + '.' + objectProperty;
                //levelOne ;
                if (node.right.type == 'ObjectExpression') {
                    var objectProperties = new Array();
                    for (var j = 0; j < node.right.properties.length; j++) {
                    ...
                    viewModelAssignmentVariable.push(new objectVariableInController(viewModelIdentifier, objectProperties, 'viewModelObject'));
                }
                if (node.right.type == 'ArrayExpression') {
                    var elements = node.right.elements;
                    var elementProperties = new Array();
                    ...
                    viewModelAssignmentVariable.push(new arrayVariableInController(viewModelIdentifier, elementProperties));
                }
                if (node.right.type == 'Literal') {
                    var variableValue = node.right.value;
                    var variableDataType = typeof variableValue;
            ...
            if (declarations[i].type == 'VariableDeclarator') {
                ...
                if (declarations[i].init) {
                    if (declarations[i].init.type == 'ObjectExpression') {
                        ...
                        variableList.push(new objectVariableInController(variableName, objectProperties, 'objectType'));
                    }
                    if (declarations[i].init.type == 'ArrayExpression') {
                        ...
                        variableList.push(new arrayVariableInController(variableName, elementProperties));
                    }
                    if (declarations[i].init.type == 'Literal') {
                        ...
                        variableList.push(new singleVariableInController(variableName, variableValue, variableDataType));
                    }
                }
                else {
